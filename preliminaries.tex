...
\begin{tabular}{l l}
	...
	$\sum$			& sum \\
	$\bigcup$		& set sum of elements\\
	$\abs{\cdot}$	& set cardinality \\
	$2^\cdot$		& family of subsets \\
...
Instead, we will revise the most important notions.

We model algorithms by an abstract \textit{Turing machine}.
...
We define \textsc{P} as a class of problems solvable by a polynomial-time Turing machine.
...
into input of another one preserving the answer.
...
we say that $A$ reduces polynomially to $B$.
...
and for each \textsc{no}-input $z$ of $A$ answers \textsc{no} to input $(z,w)$ for every $w$.
\end{defn}
...
\begin{defn}[\np-complete]
Problem $A$ belongs to \np-complete if $A$ belongs both to \textsc{NP} and \np-hard.
\end{defn}
...
\begin{rmrk}
If any \np-complete problem $A$ belongs to \textsc{P} then using remarks \ref{tran-red} and \ref{p-red}
we have that $\p = \np$.
\end{rmrk}
